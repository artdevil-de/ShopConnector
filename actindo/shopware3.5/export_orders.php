...
// Holger Ende
	...
	foreach ($orders1 as $order)
	{
		...
		$actindoorder = array(
			...
			'project_id' => 'IB-'.$order['order_number'],
			// Holger, das hier wäre die Lösung für den Zahlungsabgleich. Mindestens für PayPal und Sofortüberweisung, vermutlich für alle.
			//         Wenn actindo nun nur noch an einer Weiterentwickelung interessiert wäre ...
			'order_temporary_id' => $order['temporaryID'],
			'customer' => array(
			...
			'beleg_status_text' => trim(str_replace("'", "", !empty($order['customercomment']) ? "Onlineshop Kundenkommentar:\n---------------\n".$order['customercomment']."\n---------------\n" : '' )),
			// Holger, to-do . Bearbeiter setzen wird von actindo noch nicht unterstützt
			//'signum' => 'Herr Mustermann',
		...
		// Holger, Markierer finden und markieren
		$row0 = act_get_row("SELECT max( `aa`.`attr2` ) AS `max`FROM `s_order_details` AS `od`
                         INNER JOIN `s_articles_attributes` AS `aa` ON ( `od`.`articleID` = `aa`.`articleID` )
                         WHERE `orderID` = ".$order['orderID']." AND `modus` IN ( 0, 1 )");

		// Holger, mögliche Werte fürs Fähnchen (No, Blue, Green, Red, Purple, Orange)
		if(is_array($row0)&&isset($row0['max'])&&$row0['max']>1)
		{
			$actindoorder['beleg_status'] = 'Purple';
			$actindoorder['beleg_status_text'] .= "\nHinweis: Markiererbestellung\nLiegt ein Altersnachweis des Kunden vor?\n---------------\n";
		}
		else
		{
			$actindoorder['beleg_status'] = 'No';
		}
		...
		$verfmap = array(
			...
			'ipayment' => 'KK',
// Holger
			'paypalexpress_teams' => 'PP',
			'paypalexpress_haendler' => 'PP',
			'ipayment_teams' => 'KK',
			'ipayment_haendler' => 'KK',
...
#      }
		...
		$zahl_bedmap = array(
			...
			'paypal' => '12',
			'paypalexpress' => '12',
			'paypalexpress_teams' => '12',
			'paypalexpress_haendler' => '12',
			'ipayment' => '10',
			'ipayment_teams' => '10',
			'ipayment_haendler' => '10',
			'sofortueberweisung' => '14',
		);

		$actindoorder['customer']['zahl_bed'] = $zahl_bedmap[$actindoorder['_payment_method']];
		// Holger Ende
...
function export_orders_positions($order_id)
{
	...
	foreach ($positions1 as $pos)
	{
		...
		// ME spezial, Markierer finden und Langtext entsprechend vorbelegen
		if($pos['attr2']>1)
		{
			$pos['langtext'] .= 'Seriennummer:\n';
		}
