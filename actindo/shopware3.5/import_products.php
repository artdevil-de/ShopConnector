...
function import_product( $product )
{
  ...
  // Shopware baut das Datum in der import/shopware.php um auf YYYY-DD-MM, also drehen wir es vorher um auf dasselbe format, damit shopware es dann wieder
  // auf YYYY-MM-DD konvertiert
...
function _do_import_attributes( $data, $product, $pgruppe_translation, $default_language_id, $languages, &$i18n, $default_language_code )
{
  ...
  if( is_array($product['shop']['attributes']) )
  {
    ...
    $res = _do_set_article_attributes( $product['shop']['attributes']['combination_advanced'], $product['shop']['attributes']['names'],
      $product['shop']['attributes']['values'], $id, $product, $pgruppe_translation, $data, $default_language_id, $default_language_code );
    if( !$res['ok'] )
      return $res;
