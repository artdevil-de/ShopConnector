...
function export_products($categories_id=0, $products_id=0, $lang='', $just_list=FALSE, $from=0, $count=0x7FFFFFFF, $filters=array())
{
	...
	foreach ($products as $_idx => $p) {
		...
		$res = _do_export_attributes($p, $articleprices[(int)$p['articledetailsID']], $pgruppe_translation, $articleimages[$id]);
		if(!$res['ok'])
			return array('ok' => FALSE, 'errno' => $res['errno'], 'error' => 'Artikel '.$p['art_nr'].': '.$res['error']);
...
function _do_export_attributes(&$p, $articleprices, $pgruppe_translation, &$images)
{
	...
	$sql = "SELECT * FROM `s_articles_groups_settings` WHERE `articleID`=".(int)$id;
	if(is_object($result0 = act_db_query($sql))&&act_db_num_rows($result0)) {
		$row = act_db_fetch_assoc($result0);
		if($row['type']==3) {
			return array('ok' => FALSE, 'errno' => EINVAL, 'error' => 'Aufpreis-Konfiguratoren werden von actindo nicht unterstützt.');
		}
	}
